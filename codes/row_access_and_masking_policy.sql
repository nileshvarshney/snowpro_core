...
ALTER TABLE CUSTOMERS ADD ROW ACCESS POLICY RAP ON (EMAIL); -- this may fail as one policy is already applies
...
-- one row policy work at a time, drop previously applied and add new one.
